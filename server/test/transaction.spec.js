...
const assertError = (errorCode, amount, done, keyString, token) => chai
  .request(app)
...
const assertErrorParams = (errorCode, amount, number, done, keyString, token) => chai
  .request(app)
  .post(`${endPoint}transactions/${number}/debit`)
  .set('x-access-token', token)
  .send({ amount })
  .end((err, res) => {
    expect(res).to.have.status(errorCode);
...
describe('POST /transactions/<accout-number>/debit', () => {
  ...
  it('should return 400 if amount is omitted', (done) => {
    ...
    assertError(400, amount, done, 'kindly put in', staffToken);
  });
  ...
  it('should return 400 if amount is not a number', (done) => {
    ...
    assertError(400, amount, done, 'valid', staffToken);
  });
  ...
  it('should return 400 for insufficient funds', (done) => {
    ...
    assertError(400, amount, done, 'Insufficient', staffToken);
  });
  ...
  it('should return 400 if account number is not a number', (done) => {
    assertErrorParams(400, 12000, '13213dd', done, 'valid number', staffToken);
  });

  it('should return 400 if account number is not up to 10 digits', (done) => {
    assertErrorParams(400, 12000, 123456789, done, 'less than 10', staffToken);
  });

  it('should return 400 if account number is more than 10 digits', (done) => {
    assertErrorParams(400, 12000, 12345678912, done, 'maximum of 10', staffToken);
  });

  it('should return 404 if account number doesn\'t match any accounts', (done) => {
    assertErrorParams(404, 12000, 1234567891, done, 'doesn\'t exist', staffToken);
  });

  it('should return 400 if account is in draft phase', (done) => {
    assertErrorParams(400, 12000, 1234567801, done, 'draft', staffToken);
  });

  it('should return 400 if account is dormant', (done) => {
    assertErrorParams(400, 12000, 1234567802, done, 'dormant', staffToken);
  });
  ...
  it('should return 400 if authentication token is invalid', (done) => {
    assertErrorParams(400, 12000, userAccountNumber, done, 'invalid', invalidToken);
  });

  it('should return 401 if user token is provided (unauthorized access)', (done) => {
    assertErrorParams(401, 12000, userAccountNumber, done, 'user', userToken);
  });
  ...
  // tell the user model function for creating an account to throw an error regardless
    ...
    assertErrorParams(500, 12000, userAccountNumber, done, 'server', adminToken);
  });
});
