...
describe('POST /transactions/<accout-number>/debit', () => {
  ...
  it('should return 401 if user token is provided (unauthorized access)', (done) => {
    assertErrorParams(401, 12000, userAccountNumber, done, 'user', userToken, 'debit');
  });
});

describe('POST /transactions/<accout-number>/credit', () => {
  it('staff should be able to credit bank account', (done) => {
    const amount = {
      amount: 12300.12,
    };
    chai
  ...
  it('should return 401 if user token is provided (unauthorized access)', (done) => {
    assertErrorParams(401, 12000, userAccountNumber, done, 'user', userToken, 'credit');
  });
